...
`examples/` 是后续 SDK、adapter、E2E demo 和 conformance case 的入口。当前主产品代码已经在 `apps/desktop/` 落地，Main Runtime 位于 `apps/desktop/main/runtime/`，本目录暂不承载运行时代码，也不把计划中的示例写成已完成能力。

## 当前状态

- 暂无可直接运行的 example package。
- README 先固定示例边界、验收标准和未来目录规划。
- 示例必须以真实 `specs/` schema、generated contracts、typed preload API 或 Main Runtime API 为事实源。
- 编码 Agent 示例必须走 Main Runtime 的 coding API，不能绕过 Runtime 直接调用 OpenCode、Codex 或 Claude SDK。
...
- Contract conformance：对 `specs/fixtures`、generated TypeScript contracts 和 Main Runtime contract 做兼容性验证。
...
- 示例不能要求 Renderer 直接访问 secret、Runtime token、Provider key、文件系统或 raw IPC。
